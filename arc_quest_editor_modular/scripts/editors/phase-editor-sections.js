...
export function renderPhaseFlowSection(s, p, phaseIds, field, area, conditionOptions) {
  ...
  return `
    ...
    <div class="card">
      ...
      ${hasEnterCondition ? `
      <div class="row">
        <div class="f"><label>使用条件进入阶段 (autoEnterByCondition)</label><select data-b="ph.${s.pi}.autoStart"><option value="false" ${!p.autoStart ? 'selected' : ''}>False</option><option value="true" ${p.autoStart ? 'selected' : ''}>True</option></select></div>
        <div class="f"></div>
      </div>
