...
export function handleDeleteButtonAction(btn, state) {
    ...
    if (d.dtrcr) {
        ...
        return {
            handled: true,
            mutate: true,
            apply: () => q.collectionConfig?.rewardNodes?.[+ri]?.completionRules?.splice(+cri, 1)
        };
    }
    if (d.dtrr) {
        ...
        return {
            handled: true,
            mutate: true,
            apply: () => q.collectionConfig?.rewardNodes?.[+ri]?.rewards?.splice(+rwi, 1)
        };
    }
    if (d.dc !== undefined) {
    ...
    if (d.dcr) {
        ...
        return {
            handled: true,
            mutate: true,
            apply: () => q.collectionConfig?.categories?.[+ci]?.completionRules?.splice(+cri, 1)
        };
    }
    if (d.drn) {
        ...
        return {
            handled: true,
            mutate: true,
            apply: () => q.collectionConfig?.categories?.[+ci]?.rewardNodes?.splice(+ri, 1)
        };
    }
    if (d.drcr) {
        ...
        return {
            handled: true,
            mutate: true,
            apply: () => q.collectionConfig?.categories?.[+ci]?.rewardNodes?.[+ri]?.completionRules?.splice(+cri, 1)
        };
    }
    if (d.drr) {
        ...
        return {
            handled: true,
            mutate: true,
            apply: () => q.collectionConfig?.categories?.[+ci]?.rewardNodes?.[+ri]?.rewards?.splice(+rwi, 1)
        };
    }
    if (d.dr) {
        ...
        return {
            handled: true,
            mutate: true,
            apply: () => (scope === 'quest' ? q.rewards : q.phases[+phaseIndex].rewards).splice(+itemIndex, 1)
        };
    }
    if (d.do !== undefined) {
        return {
