...
function buildGraph(phases) {
  const map = new Map();
  const edges = [];
  phases.forEach((p, i) => map.set(p.id, { id: p.id, index: i, p, inDegree: 0, children: [], depth: 0, x: 0, y: 0, width: 240, height: 80 + Math.min((p.objectives?.length || 0) * 24, 96) }));
  phases.forEach(p => {
...
function edgeSvg(e, orientation) {
  ...
  let dash = '';
  if (e.type === 'parallel') { stroke = 'var(--parallel)'; dash = 'stroke-dasharray="6 4"'; }
  if (e.type === 'choice') { stroke = 'var(--choice)'; dash = 'stroke-dasharray="3 4"'; }
...
function nodeHtml(n, sel, err) {
  ...
  let objs = '';
  (n.p.objectives || []).slice(0, 3).forEach(o => { objs += `<div class="node-obj"><span></span> ${esc(o.type)}</div>`; });
...
export function bindGraphEvents(viewport, state, onClick) {
  ...
  let startX = 0;
  let startY = 0;
  const update = () => { canvas.style.transform = `translate(${view.x}px, ${view.y}px) scale(${view.k})`; };
  const fit = () => {
    const nodes = canvas.querySelectorAll('.node-fo');
    if (!nodes.length) return;
    let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
    nodes.forEach(n => {
      const x = parseFloat(n.getAttribute('x')), y = parseFloat(n.getAttribute('y')), w = parseFloat(n.getAttribute('width')), h = parseFloat(n.getAttribute('height'));
  ...
  const reset = () => { view.x = 0; view.y = 0; view.k = 1; fit(); };
  const setHundred = () => { view.k = 1; update(); };
  if (view.x === 0 && view.y === 0 && view.k === 1) fit(); else update();
  viewport.__graphApi = { fitToView: fit, updateTransform: update, resetView: reset };
  viewport.addEventListener('mousedown', e => { if (e.target.closest('.graph-controls') || e.target.closest('.node-fo')) return; dragging = true; startX = e.clientX - view.x; startY = e.clientY - view.y; viewport.style.cursor = 'grabbing'; });
  window.addEventListener('mousemove', e => { if (!dragging) return; view.x = e.clientX - startX; view.y = e.clientY - startY; requestAnimationFrame(update); });
  window.addEventListener('mouseup', () => { dragging = false; viewport.style.cursor = 'grab'; });
  viewport.addEventListener('wheel', e => { e.preventDefault(); const r = viewport.getBoundingClientRect(), px = e.clientX - r.left, py = e.clientY - r.top; const tx = (px - view.x) / view.k, ty = (py - view.y) / view.k; view.k = Math.max(.1, Math.min(3, view.k * (e.deltaY > 0 ? .85 : 1.15))); view.x = px - tx * view.k; view.y = py - ty * view.k; update(); }, { passive: false });
  viewport.querySelector('.g-scale-100').onclick = e => { e.stopPropagation(); setHundred(); };
  viewport.querySelector('.g-fit').onclick = e => { e.stopPropagation(); fit(); };
  viewport.querySelector('.g-reset').onclick = e => { e.stopPropagation(); reset(); };
  const edges = canvas.querySelectorAll('.edge-path'), nodes = canvas.querySelectorAll('.node-fo');
  nodes.forEach(node => {
    node.onclick = e => { e.stopPropagation(); onClick(Number(node.dataset.index)); };
    node.onmouseenter = () => {
      const id = node.dataset.id;
      nodes.forEach(n => { if (n !== node) n.style.opacity = '.3'; });
      edges.forEach(edge => {
        if (edge.dataset.source === id || edge.dataset.target === id) {
          edge.style.opacity = '1'; edge.style.strokeWidth = '4';
    ...
    node.onmouseleave = () => { nodes.forEach(n => n.style.opacity = '1'); edges.forEach(edge => { edge.style.opacity = '1'; edge.style.strokeWidth = '2.5'; }); };
  });
}
...
export function bindGraphShellEvents(shell, state, onClick) {
  ...
  const close = () => { state.ui.graphExpanded = false; overlay.classList.remove('open'); document.body.classList.remove('graph-expanded-open'); };
  const open = () => { state.ui.graphExpanded = true; body.innerHTML = viewportHtml(state, 'graph-viewport-expanded'); placeOverlay(shell, overlay); overlay.classList.add('open'); document.body.classList.add('graph-expanded-open'); const vp = body.querySelector('.graph-viewport-expanded'); if (vp) { bindGraphEvents(vp, state, onClick); requestAnimationFrame(() => vp.__graphApi?.fitToView()); } };
  shell.querySelector('[data-graph-toggle="expand"]').onclick = e => { e.stopPropagation(); open(); };
  overlay.querySelector('[data-graph-backdrop]').onclick = close;
  overlay.querySelector('[data-graph-toggle="collapse"]').onclick = e => { e.stopPropagation(); close(); };
  if (!overlay.__resizeBound) { overlay.__resizeBound = true; window.addEventListener('resize', () => { if (!overlay.classList.contains('open')) return; const activeShell = document.querySelector('.graph-shell'); if (!activeShell) return; placeOverlay(activeShell, overlay); overlay.querySelector('.graph-viewport-expanded')?.__graphApi?.fitToView(); }); }
}
