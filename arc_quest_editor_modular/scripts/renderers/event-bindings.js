...
export function bindEditorActions(midEl, state, rerender, setByPath) {
  ...
  midEl.onclick = e => {
    ...
    if (id === 'addCategoryBtn') { q.collectionConfig ||= { categories: [], rewardNodes: [], completionRules: [] }; q.collectionConfig.categories.push(createCollectionCategory(q.collectionConfig.categories.length)); }
    ...
    if (id === 'addTopRewardNodeBtn') { q.collectionConfig ||= { categories: [], rewardNodes: [], completionRules: [] }; q.collectionConfig.rewardNodes ||= []; q.collectionConfig.rewardNodes.push(createTopRewardNode(q.collectionConfig.rewardNodes.length)); }
    ...
    if (id === 'deletePhaseBtn') { if (q.phases.length <= 1) return alert('至少保留一个 phase。'); if (!confirm(`确定删除 phase: ${q.phases[s.pi].id} 吗？`)) return; q.phases.splice(s.pi, 1); state.ui.sel = { t: 'quest' }; }
