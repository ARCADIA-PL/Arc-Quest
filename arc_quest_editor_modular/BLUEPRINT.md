...
## 十一、对话编辑器 UX 终极升级（4 人设计师 · 20 轮研讨）

> 研讨日期: 2026-05-15
> 参与方: Leah (信息架构师) / Ren (交互设计师) / Sasha (视觉设计师) / Marcus (系统设计师)
> 背景: 第二次 UX 评审 — 当前方向正确但编辑不顺手，需全方位升级
...
| # | 变更 | 理由 |
|---|------|------|
| 1 | **废弃 Say 编辑页** (删除 `dialogue-say-editor.js`) | speaker + text 是节点属性而非独立语义层，应在节点详情页直接编辑 |
| 2 | **保留 sayIf/choice 独立编辑页** | 保持心智分层，避免把条件文本/选项编辑表单堆回节点页 |
| 3 | **节点摘要页 → 节点详情页** (重写 `dialogue-node-summary.js`) | 整合 speaker/text 可编辑字段 + sayIf/choice 增强摘要卡片 |
| 4 | **左栏树简化** — 只显示 config + 节点列表 | sayIf/choice 不再作为左栏子项；折叠控制权移入节点详情页分区块 |
| 5 | **state.ui.sel 移除 `say` 类型** | 五个值精简为四个: `config` / `node` / `sayIf` / `choice` |
| 6 | **条件编辑器局部重渲染** | 不再触发全页 rerender, 防止滚动跳顶和焦点丢失 |
...
dialogue.ui: {
    ...
    //  或 { t: 'sayIf', ni: 0, key: 'newcomer' }
    //  或 { t: 'choice', ni: 0, ci: 2 }
    sayIfFold: true,   // 节点详情页 SayIf 区折叠（不再控制左栏）
    choiceFold: true,  // 节点详情页 Choice 区折叠
}
```
...
│ ┌─ SayIf (8) ──────────────────── [＋] [▾ 展开] ┐│
│ │ ┌─ newcomer ────────────────────── pri:0 ─┐  ││
│ │ │ [NOT] [has_quest: epic_prologue]        │  ││
│ │ │ "欢迎来到村庄，旅行者。我是这里的村..."  │  ││
│ │ │                          [编辑] [▲] [▼] │  ││
│ │ └─────────────────────────────────────────┘  ││
│ │ ┌─ phase_gather ────────────────── pri:0 ─┐  ││
│ │ │ [quest_phase: epic_prologue, gather_wood]│  ││
│ │ │ "快去收集木材，村口需要加固..."          │  ││
│ │ │                          [编辑] [▲] [▼] │  ││
│ │ └─────────────────────────────────────────┘  ││
│ └────────────────────────────────────────────────┘│
...
| # | 升级 | 细节 |
|---|------|------|
| 1 | **同级导航** | sayIf/choice 编辑页顶部 `[◀ prev] [next ▶]` + `< 3/8 >` 计数, 遍历同级条目不离开编辑页 |
| 2 | **面包屑** | 节点详情页 `⚙ 对话配置 > start` 可点击回 config; sayIf/choice 页 `← 节点详情: start` |
| 3 | **条件 chip 彩色语义** | `has_flag`=绿色, `not_has_flag`=红色, `quest_phase`=蓝色, `quest_completed`=紫色, `not`=灰色, `and/or`=黄色 |
| 4 | **sayIf/choice 摘要卡片** | 条件 chip 彩色 + 文本预览 60 字符 + ✕ 删除按钮 + `[编辑]` 右对齐 |
| 5 | **分区折叠** | 节点详情页 SayIf 区和 Choice 区各有独立展开/收起按钮, 默认展开 |
| 6 | **条件编辑器局部重渲染** | `bindConditionEditorClicks` 新增 `skipRerender` 参数, dialogue 模式直接操作 DOM 追加条件节点, 不触发全页 rerender → 消除滚动跳顶 |
...
| # | CSS 类 | 用途 |
|---|--------|------|
| 1 | `.breadcrumb` | 顶部面包屑: `⚙ 对话配置  >  start` |
| 2 | `.cond-chip` | 条件彩色徽标: `flag=#32D74B` `quest=#0A84FF` `not=#8E8E93` `and/or=#FF9F0A` |
| 3 | `.detail-card` | 节点详情页 compact 卡片, 替代 `.summary-row` |
...
| 操作 | 文件 | 内容 |
|------|------|------|
| **重写** | `dialogue-node-summary.js` | → 节点详情页 (speaker/text 可编辑 + sayIf/choice 增强摘要卡片 + 分区折叠) |
| **修改** | `dialogue-sayif-editor.js` | 同级导航 prev/next + 面包屑 + 条件局部重渲染 |
| **修改** | `dialogue-choice-editor.js` | 同级导航 prev/next + 面包屑 + 条件局部重渲染 |
| **修改** | `dialogue-tree-renderer.js` | 左栏简化 (只显示 config + 节点列表, 移除 sayIf/choice 子项) |
| **修改** | `dialogue-click-actions.js` | 新增 sayIf/choice 同级导航 + 条件局部重渲染跳过 rerender |
| **修改** | `dialogue-editor.js` | center 路由移除 `say` 分支 + renderDialogueConfig 面包屑 |
| **修改** | `state.js` | sel 移除 `say` 类型 |
| **修改** | `app.js` | `ensureValidDialogueSelection` 移除 `say` 分支 + 左栏树点击移除 say |
| **修改** | `editor.css` | `.breadcrumb` / `.cond-chip` / `.detail-card` 样式 |
| **删除** | `dialogue-say-editor.js` | 废弃 (speaker/text 移到节点详情页) |
| **修改** | `editor-click-actions-condition.js` | 新增 `skipRerender` 参数, dialogue 模式局部 DOM 更新 |
...
| # | 决策 | 理由 |
|---|------|------|
| 1 | sayIf/choice 保持独立编辑页 | 心智分层清晰, 避免信息过载 |
| 2 | say 编辑页废弃 | speaker/text 是节点属性, 不应跨页编辑 |
| 3 | 节点详情页可编辑 speaker/text | 减少一层跳转, 对齐 quest phase 编辑页模式 |
| 4 | 同级导航替代"返回再选" | 编辑多个 sayIf 时无需反复返回节点页 |
| 5 | 面包屑替代返回按钮 | 提供空间位置感知, 减少按钮视觉噪音 |
| 6 | 条件 chip 彩色语义 | 视觉区分 7 种条件类型, 一行扫读 |
| 7 | 左栏只显示到节点层 | sayIf/choice 遍历在节点详情页内完成, 不撑满左栏 |
| 8 | 分区折叠独立控制 | 节点作者可能只关心 sayIf 不关心 choice, 反之亦然 |
| 9 | 条件编辑器局部重渲染 | 消除滚动跳顶, 保持编辑焦点 |
| 10 | 文本预览 60 字符 (原 40) | 减少截断, 中文平均 60 字符约 2 行信息 |
| 11 | ✕ 删除放摘要卡片右上角 | 视觉惯例, 不占用底部操作区 |
| 12 | ▲▼ 优先级排序按钮后做 | 需要改 state 结构支持排序, 先不纳入 |
| 13 | 节点总览页（网格视图）后做 | 适用于 20+ 节点的大型对话, 当前对话规模不需要 |
| 14 | speaker 默认值功能后做 | 须先确认 Java runtime fallback 链, 避免 UI 行为与运行时不符 |
| 15 | 条件编辑器彻底重构（Web Component）后做 | 需改变渲染模型, 成本高, 当前缓解方案够用 |
| 16 | 左栏虚拟滚动后做 | 30+ 节点时 DOM 性能优化, 当前规模不需要 |
