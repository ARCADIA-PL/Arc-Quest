...
Arc Quest 包含一个高度可定制的交易系统，支持通过代码定义商店、分类和具体的交易项。

### 核心特性
1. **多货币/多物品支持**: 支持任意类型的花费（`ITradeOffer`），如物品、经验、计分板等。
2. **多奖励支持**: 购买后可以执行命令、给予物品、设置标志位。
3. **前置条件与限制**: 每个交易项都可以配置前置条件（任务完成、标志位）以及最大购买次数。
4. **冷却机制**: 独立配置商店或单个商品的刷新冷却（游戏日、游戏刻、现实时间）。
