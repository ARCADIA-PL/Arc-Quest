...
public class TradeGridPanel {
    ...
    public void updateHoverAnimsSize(int size) { this.hoverAnims = new float[size]; }
    public void setHoverAnim(int index, float val) { if(index >= 0 && index < hoverAnims.length) hoverAnims[index] = val; }
    ...
    private float easeOutBack(float t) { float f = t - 1f; return 1f + 2.70158f * (f * f * f) + 1.70158f * (f * f); }
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            if (gi == screen.getLastClickedGi()) {
                if (screen.isFeedbackSuccess() && screen.getFeedbackAnim() > 0) {
                    bdA = Math.min(255, bdA + (int)(170 * screen.getFeedbackAnim() * alpha)); bRgb = screen.lerpColor(bRgb, 0x55FF55, screen.getFeedbackAnim());
                    ...
                    bdA = Math.min(255, bdA + (int)(170 * intensity * alpha)); bRgb = screen.lerpColor(bRgb, 0xFF3333, intensity);
                }
            }

            g.pose().pushPose(); g.pose().translate(drawX + l.cardW()/2f, drawY + l.cardH()/2f, 0); g.pose().scale(bgScale, bgScale, 1f); g.pose().translate(-(drawX + l.cardW()/2f), -(drawY + l.cardH()/2f), 0);
            ...
            if (contentScale > 0.01f) {
                g.pose().pushPose(); g.pose().translate(drawX + l.cardW()/2f, drawY + l.cardH()/2f, 0); g.pose().scale(contentScale, contentScale, 1f); g.pose().translate(-(drawX + l.cardW()/2f), -(drawY + l.cardH()/2f), 0);

                int itemDrawY = (int)drawY + (l.cardH() - 16) / 2;
                if (entry.getRewardIcon() != null) screen.drawAdaptiveIcon(g, entry.getRewardIcon(), (int)drawX + 6, itemDrawY, 16, 16, clampedEase * alpha);
                else { ItemStack icon = screen.getIconStackForEntry(entry); if (!icon.isEmpty()) g.renderItem(icon, (int)drawX + 6, itemDrawY); }
                ...
                if (!statusStr.isEmpty()) g.drawString(font, statusStr, (int)drawX + 26 + font.width(nameStr) + 4, textY, HudAnimUtil.withAlpha(scColor, (int)(255 * clampedEase * alpha)), true);

                int cX = (int)drawX + 26, costY = textY + font.lineHeight + 4;
                for (int j = 0; j < entry.getCosts().size(); j++) {
                    if (cX - ((int)drawX + 26) > l.cardW() - 49) { g.drawString(font, "...", cX, costY, HudAnimUtil.withAlpha(0xFFFFFF, (int)(255 * clampedEase * alpha)), true); break; }
                    ITradeOffer cost = entry.getCosts().get(j);
                    if (j > 0) { g.drawString(font, "+", cX, costY, HudAnimUtil.withAlpha(0x777777, (int)(255 * clampedEase * alpha)), true); cX += font.width("+") + 2; }

                    g.pose().pushPose(); g.pose().translate(cX, costY - 1, 0); g.pose().scale(0.6f, 0.6f, 1f);
                    if (cost.getIcon() != null) screen.drawAdaptiveIcon(g, cost.getIcon(), 0, 0, 16, 16, clampedEase * alpha);
                    ...
                    if (font.width(cDesc) > (l.cardW() - 34) - (cX - ((int)drawX + 26))) cDesc = font.plainSubstrByWidth(cDesc, Math.max(1, (l.cardW() - 34) - (cX - ((int)drawX + 26)) - 6)) + "..";
                    g.drawString(font, cDesc, cX, costY, HudAnimUtil.withAlpha(canBuy ? screen.getThemeColorForEntry(entry) : 0x777777, (int)(255 * clampedEase * alpha)), true); cX += font.width(cDesc) + 4;
                }
                g.pose().popPose();
            }
        }
