...
public class JournalDetailRewards {
    ...
    public int render(GuiGraphics g, QuestDefinition def, String selectedPhaseId, int x, int scrollAreaY, int scrollAreaW, int scrollAreaH, int mx, int my, int activeTheme, float dAlpha, int safeA, int localY, float dt) {
        ...
        if (selectedPhaseId != null && !selectedPhaseId.isEmpty()) {
            ...
            if (phase != null && !phase.getPhaseRewards().isEmpty()) phaseRewards = phase.getPhaseRewards();
        }
        ...
        // --- 核心双通道：延迟渲染 3D 物品任务队列 ---
        List<Runnable> pass2ItemRenders = new ArrayList<>();
        int itemX = 0;
        for (IReward r : currentRewards) {
            int rW = getRewardWidth(r, font), rH = 24;
            RewardCache cachedReward = getRewardCache(r, font);
            int absHitX = x + 12 + renderStartX + itemX, absHitY = absItemsY;

            boolean inBounds = !needsScissor || (absHitX + rW >= x + 12 && absHitX <= x + 12 + localW);
            boolean hovered = inBounds && mx >= absHitX && mx <= absHitX + rW && my >= absHitY && my <= absHitY + rH && my >= parentClipY1 && my <= parentClipY2;
            ...
            if (r instanceof ItemReward) {
                ...
                if (inBounds && itemDAlpha > 0.01f) {
                    ...
                    pass2ItemRenders.add(() -> {
                        g.pose().pushPose();
                        float itemCx = finalItemX + rW / 2f, itemCy = rH / 2f - 2f;
        ...
        // == PASS 2: 集中渲染 3D 物品（避免反复切换着色器和深度缓冲区）==
        if (!pass2ItemRenders.isEmpty()) {
            for (Runnable task : pass2ItemRenders) task.run();
        }
