...
public class JournalDetailPanel {
    ...
    private static class DetailHeaderCache {
        String questId = ""; int descWidth = -1; String titleText = ""; int titleWidth = 0; String descriptionText = ""; List<String> descriptionLines = List.of();
        void clear() { questId = ""; descWidth = -1; titleText = ""; titleWidth = 0; descriptionText = ""; descriptionLines = List.of(); }
    }
    ...
    public static void drawCyberButton(GuiGraphics g, QuestJournalScreen screen, int x, int y, int w, int h, String text, int themeColor, float hoverEase, boolean hovered) {
        ...
        if (screen.getEffectiveAlpha() > 0.05f) {
            int textW = screen.getFont().width(text); float baseScale = 0.85f;
    ...
    public static boolean shouldShowBranchChoices(QuestDefinition def, QuestRuntimeData runtime, String phaseId) {
        ...
        PhaseDefinition phase = def.getPhase(phaseId); if (phase == null || !phase.hasChoices()) return false;
    ...
    public void resetState() { detailReveal = 0f; detailTargetScroll = 0; detailScrollOffset = 0; headerCache.clear(); singlePhaseRenderer.reset(); parallelPhaseRenderer.reset(); }
    ...
    public void render(GuiGraphics g, int x, int y, int w, int h, int mx, int my, int theme, float dt) {
        ...
        if (selectedIndex < 0 || selectedIndex >= screen.getCurrentEntries().size()) { renderEmptyDetail(g, x, y, w, h); return; }

        JournalTypes.QuestListEntry entry = screen.getCurrentEntries().get(selectedIndex); QuestDefinition def = entry.def(); if (def == null) return;
        int activeTheme = ClientQuestCache.INSTANCE.getQuestThemeColor(entry.questId(), theme); screen.setCurrentThemeColor(activeTheme);
        ...
        int safeA = (int) (255 * dAlpha); if (safeA <= 8) return;
        ...
        def.getSplashConfig(SplashType.QUEST_DETAIL).ifPresent(asset -> {
            RenderSystem.enableBlend(); float watermarkAlpha = 0.15f * dAlpha; int rw = (int) (w * 0.7f), rh = rw;
            int rx = x + w / 2 - rw / 2 + (int) ((1f - detailReveal) * 50f), ry = scrollAreaY + scrollAreaH / 2 - rh / 2;
            RenderSystem.setShaderColor(1f, 1f, 1f, watermarkAlpha); QuestIconRenderer.renderIcon(g, asset, rx, ry, rw, rh); RenderSystem.setShaderColor(1f, 1f, 1f, 1f);
        });

        g.pose().pushPose(); g.pose().translate(x + 12, scrollAreaY + 12 - detailScrollOffset, 0);
        ...
        if (def.getVisualConfig().getIcon(IconPosition.QUEST_TITLE).isPresent()) {
            ...
            def.getVisualConfig().getIcon(IconPosition.QUEST_TITLE).ifPresent(icon -> {
                RenderSystem.setShaderColor(1f, 1f, 1f, dAlpha); QuestIconRenderer.renderIcon(g, icon, 0, finalLocalY - 2, 16, 16); RenderSystem.setShaderColor(1f, 1f, 1f, 1f);
            });
            titleIconOffset = 22;
        }

        DetailHeaderCache header = getHeaderCache(entry, def, scrollAreaW); String titleText = header.titleText;
        g.pose().pushPose(); g.pose().translate(titleIconOffset, localY, 0); g.pose().scale(1.2f, 1.2f, 1f);
        ...
        g.pose().pushPose(); g.pose().translate(hBtnX, hBtnY, 0); g.pose().mulPose(Axis.ZP.rotationDegrees(45));
        ...
        historyBtnRect[0] = absBtnX - hBtnR - 4; historyBtnRect[1] = absBtnY - hBtnR - 4; historyBtnRect[2] = hBtnR * 2 + 8; historyBtnRect[3] = hBtnR * 2 + 8;
        ...
        if (remainSec >= 0) {
            String timeStr = formatAsClock(remainSec); float timeScale = 1.0f;
            int timerRenderW = (int) (screen.getFont().width(timeStr) * timeScale);
            float pulse = 1.0f; int activeTimerColor = activeTheme;
            if (remainSec <= 60) { pulse = 0.6f + 0.4f * (float) Math.sin(Util.getMillis() / (remainSec <= 10 ? 80.0 : 200.0)); if (remainSec <= 10) activeTimerColor = 0xFF4444; }
            int timeColor = HudAnimUtil.withAlpha(activeTimerColor, (int)(safeA * pulse)), timeX = (scrollAreaW - 24) - timerRenderW;
            int minTimerX = hBtnX + hBtnR + 16; if (timeX < minTimerX) timeX = minTimerX; int timeY = localY + 2;
            g.pose().pushPose(); g.pose().translate(timeX, timeY, 0); g.pose().scale(timeScale, timeScale, 1f);
            g.drawString(screen.getFont(), timeStr, 0, 0, timeColor, true); g.pose().popPose();
        }

        localY += 18;
        if (!header.descriptionText.isEmpty()) {
            g.pose().pushPose(); g.pose().translate(0, localY, 0); g.pose().scale(0.85f, 0.85f, 1f);
            for (String line : header.descriptionLines) { g.drawString(screen.getFont(), line, 0, 0, HudAnimUtil.withAlpha(0xAAAAAA, safeA), false); g.pose().translate(0, screen.getFont().lineHeight + 1, 0); }
            g.pose().popPose(); localY += header.descriptionLines.size() * (int) (screen.getFont().lineHeight * 0.85f + 1) + 8;
        }

        g.fill(0, localY, scrollAreaW - 24, localY + 1, HudAnimUtil.withAlpha(activeTheme, (int) (120 * dAlpha))); localY += 10;
        ...
        else if (entry.state() == QuestState.FAILED) { g.drawString(screen.getFont(), questFailedText, 0, localY, HudAnimUtil.withAlpha(0xFF6666, safeA), false); localY += 16; }
        ...
        detailContentHeight = localY + 12; g.pose().popPose(); g.disableScissor();
    ...
    private DetailHeaderCache getHeaderCache(JournalTypes.QuestListEntry entry, QuestDefinition def, int scrollAreaW) {
        int descWidth = (int) ((scrollAreaW - 24) / 0.85f); String description = def.getDescription().getString();
        if (!entry.questId().equals(headerCache.questId) || headerCache.descWidth != descWidth || !description.equals(headerCache.descriptionText)) {
            headerCache.questId = entry.questId(); headerCache.descWidth = descWidth; headerCache.titleText = def.getDisplayName().getString();
            headerCache.titleWidth = screen.getFont().width(headerCache.titleText); headerCache.descriptionText = description;
            headerCache.descriptionLines = description.isEmpty() ? List.of() : HudRenderUtil.wrapText(description, descWidth, screen.getFont());
        }
        return headerCache;
    }
    private void renderEmptyDetail(GuiGraphics g, int x, int y, int w, int h) { if (screen.getEffectiveAlpha() > 0.05f) g.drawCenteredString(screen.getFont(), selectQuestText, x + w / 2, y + h / 2, HudAnimUtil.withAlpha(0x666666, (int) (120 * screen.getEffectiveAlpha()))); }
    ...
    public boolean mouseClicked(double mx, double my, int x, int y, int w, int h) {
        ...
        if (screen.getSelectedIndex() >= 0 && screen.getSelectedIndex() < screen.getCurrentEntries().size()) {
            JournalTypes.QuestListEntry entry = screen.getCurrentEntries().get(screen.getSelectedIndex()); QuestRuntimeData runtime = ClientQuestCache.INSTANCE.getActiveQuest(entry.questId());
            if (entry.state() == QuestState.ACTIVE && runtime != null) {
                List<String> activePhaseIds = new ArrayList<>(); for (String pid : entry.def().getPhaseIds()) if (runtime.isPhaseActive(pid)) activePhaseIds.add(pid);
                if (activePhaseIds.isEmpty()) activePhaseIds.addAll(runtime.getActivePhaseIds());
                if (activePhaseIds.size() == 1) { if (singlePhaseRenderer.mouseClicked(mx, my, x, y, w, h)) return true; }
                else if (activePhaseIds.size() > 1) { if (parallelPhaseRenderer.mouseClicked(mx, my, x, y, w, h)) return true; }
            }
        }
        return false;
    }
    public boolean mouseDragged(double mx, double my, int y, int h) {
        ...
        if (isDraggingDetailScrollbar) { updateScrollFromMouse(my, y, h - 40, Math.max(0, detailContentHeight - (h - 40))); return true; }
        return parallelPhaseRenderer.mouseDragged(mx, my);
    }
    public boolean mouseReleased(int button) { if (rewardsRenderer.mouseReleased(button)) return true; if (button == 0) { isDraggingDetailScrollbar = false; parallelPhaseRenderer.onMouseReleased(); } return isDraggingDetailScrollbar; }
    public boolean mouseScrolled(double mx, double my, double delta, int x, int y, int w, int h) {
        ...
        if (mx >= x && mx <= x + w && my >= y && my <= y + h) { detailTargetScroll -= delta * 25.0; clampScroll(h - 40); return true; } return false;
    }
    ...
    private long getQuestRemainSeconds(QuestDefinition def, QuestRuntimeData runtime) {
        ...
        if (type == QuestTimeLimitType.REAL_SECONDS) { long accepted = runtime.getAcceptedAtRealMs(); if (accepted <= 0L) return -1L; long elapsedSec = Math.max(0L, (System.currentTimeMillis() - accepted) / 1000L); return Math.max(0L, limit - elapsedSec); }
        else if (type == QuestTimeLimitType.GAME_DAY_TIME) { long acceptedDay = runtime.getAcceptedAtDayTime() % 24000L; long nowDay = (screen.getMinecraft().level != null) ? (screen.getMinecraft().level.getDayTime() % 24000L) : acceptedDay; long elapsedTicks = (nowDay - acceptedDay + 24000L) % 24000L; long remainTicks = Math.max(0L, limit - elapsedTicks); return remainTicks / 20L; } return -1L;
    }
    private String formatAsClock(long totalSeconds) { long s = Math.max(0L, totalSeconds), h = s / 3600L, m = (s % 3600L) / 60L, sec = s % 60L; if (h > 0L) return String.format("%02d:%02d:%02d", h, m, sec); return String.format("%02d:%02d", m, sec); }
}