...
public final class DialogueHistoryPanel {
    ...
    public static void toggle() { if (active && !closing) close(); else open(); }

    public static void open() {
        active = true; closing = false; enterTimer = 0f; exitTimer = 0f;
        isDraggingScrollbar = false; isDraggingContent = false;
        renderBlockCache = List.of(); renderBlockSignature = Long.MIN_VALUE;
        renderBlockWidth = -1; renderBlockTotalHeight = 0;
    ...
    public static void close() {
        ...
        closing = true; exitTimer = 0f; isDraggingScrollbar = false; isDraggingContent = false;
    }

    public static boolean keyPressed(int keyCode) {
        if (!active || closing) return false;
        if (keyCode == 256 || keyCode == 72) { close(); return true; }
        return false;
    }
    ...
    public static boolean mouseClicked(double mx, double my, int button) {
        ...
        if (localX < 0 || localX > PANEL_W || localY < 0 || localY > PANEL_H) { close(); return true; }
        ...
        if (maxScroll > 0) {
            ...
            if (localX >= PANEL_W - 15 && localY >= contentYStart && localY <= contentYStart + viewHeight) {
                isDraggingScrollbar = true; updateScrollbarDrag(localY); return true;
            } else if (localY >= contentYStart && localY <= contentYStart + viewHeight) {
                isDraggingContent = true; dragStartMouseY = (float) my; dragStartScrollOffset = targetScrollOffset; return true;
            }
        }
        return true;
    }
    ...
    public static boolean mouseDragged(double mx, double my, int button, double dragX, double dragY) {
        if (!active || closing) return false;
        if (isDraggingScrollbar) { updateScrollbarDrag((float) (my - currentDrawY) / currentScale); return true; }
        if (isDraggingContent) {
            targetScrollOffset = Math.max(0, Math.min(dragStartScrollOffset - ((float) (my - dragStartMouseY)) / currentScale, maxScroll));
            scrollOffset = targetScrollOffset; return true;
        }
        return false;
    }
    ...
    public static boolean mouseReleased(double mx, double my, int button) {
        if (!active || closing || button != 0) return false;
        if (isDraggingScrollbar || isDraggingContent) { isDraggingScrollbar = false; isDraggingContent = false; return true; }
        return false;
    }

    private static void forceScrollToBottom() { scrollOffset = 99999f; targetScrollOffset = 99999f; }
    ...
    public static void render(GuiGraphics g, int mx, int my, float partialTick) {
        ...
        long now = System.currentTimeMillis(); float dt = Math.min((now - lastRenderMs) / 1000f, 0.1f); lastRenderMs = now;
        ...
        if (closing) {
            exitTimer += dt; if (exitTimer >= EXIT_TIME) { active = false; closing = false; return; }
            float t = Math.min(1.0f, exitTimer / EXIT_TIME);
            wipeProgress = (float) Math.pow(t, 4.0); currentY = baseY + (wipeProgress * actualFlyDist * 2f);
            ...
            revealProgress = easeOut; alphaF = easeOut;
            scaleAnim = finalScale * (0.95f + 0.05f * easeOut); currentY = baseY + (1.0f - easeOut) * actualFlyDist * 2f;
        }

        if (revealProgress <= 0.001f || wipeProgress >= 0.999f) return;

        float drawWidth = PANEL_W * scaleAnim, drawHeight = PANEL_H * scaleAnim;
        currentDrawX = baseX - (drawWidth - PANEL_W * finalScale) / 2f; currentDrawY = currentY - (drawHeight - PANEL_H * finalScale) / 2f;
        ...
        g.pose().pushPose(); g.pose().translate(0, 0, 5000);
        ...
        g.pose().pushPose(); g.pose().translate(currentDrawX, currentDrawY, 0); g.pose().scale(scaleAnim, scaleAnim, 1f);

        // === PASS 1: 面板纯 2D 批处理 ===
        renderPanel(g, mc.font, Math.max(0, Math.min(255, (int) (255 * alphaF))), alphaF, dt);

        g.pose().popPose(); g.disableScissor(); g.pose().popPose();
    }
    ...
    private static void renderPanel(GuiGraphics g, Font font, int alpha, float alphaF, float dt) {
        ...
        g.fill(0, 0, 15, 1, HudAnimUtil.withAlpha(THEME_COLOR, alpha)); g.fill(0, 0, 1, 15, HudAnimUtil.withAlpha(THEME_COLOR, alpha)); g.fill(PW - 15, 0, PW, 1, HudAnimUtil.withAlpha(THEME_COLOR, alpha));
        ...
        if (transcript.size() != lastEntryCount) { lastEntryCount = transcript.size(); forceScrollToBottom(); }
        ...
        if (targetScrollOffset > maxScroll) targetScrollOffset = maxScroll; if (scrollOffset > maxScroll) scrollOffset = maxScroll;
        if (!isDraggingScrollbar && !isDraggingContent) scrollOffset += (targetScrollOffset - scrollOffset) * Math.min(1f, dt * 15f);
        ...
        for (RenderBlock block : blocks) {
            if (currentY + block.height < contentYStart + scrollOffset || currentY > contentYStart + scrollOffset + viewHeight) { currentY += block.height; continue; }
            ...
            for (String line : block.lines) { g.drawString(font, line, leftX + 12, textY, HudAnimUtil.withAlpha(textColor, alpha), false); textY += font.lineHeight + 6; }
    ...
    private static List<RenderBlock> getRenderBlocks(List<TranscriptEntry> transcript, int wrapWidth, Font font) {
        ...
        List<RenderBlock> blocks = new ArrayList<>(transcript.size()); int totalHeight = 0;
        for (TranscriptEntry entry : transcript) {
            boolean isPlayer = "player".equalsIgnoreCase(entry.role());
            RenderBlock block = new RenderBlock(); block.isPlayer = isPlayer; block.speaker = (entry.speaker() == null || entry.speaker().isBlank()) ? (isPlayer ? "YOU" : "UNKNOWN") : entry.speaker();
            block.lines = HudRenderUtil.wrapText(entry.text(), wrapWidth, font); block.height = 14 + (block.lines.size() * (font.lineHeight + 6)) + 16;
            blocks.add(block); totalHeight += block.height;
        }
        renderBlockSignature = compactedContentSignature; renderBlockWidth = wrapWidth; renderBlockCache = blocks; renderBlockTotalHeight = totalHeight; return blocks;
    }

    private static List<TranscriptEntry> compactTranscript(List<TranscriptEntry> source) {
        if (source == null || source.isEmpty()) return List.of();
        List<TranscriptEntry> compact = new ArrayList<>(source.size()); EntryKey lastKey = null;
        for (TranscriptEntry e : source) {
            if (e == null) continue; EntryKey curKey = EntryKey.of(e);
            if (lastKey != null && lastKey.equals(curKey)) continue;
            compact.add(e); lastKey = curKey;
        }
        return compact;
    }
    ...
    private static void drawHorizontalCyberBase(GuiGraphics g, int startX, int endX, int bottomY, int themeColor, float alphaPercentage) {
        if (alphaPercentage < 0.02f) return; int alpha = (int) (255 * alphaPercentage); if (alpha < 5) return;
    ...
    private static class RenderBlock { boolean isPlayer; String speaker; List<String> lines; int height; }

    private static void ensureCompactedTranscriptUpToDate(List<TranscriptEntry> raw) {
        int currentSize = (raw == null) ? 0 : raw.size(); long currentSig = rollingSignature(raw);
        if (currentSize == compactedSourceSize && currentSig == compactedContentSignature) return;
        compactedSourceSize = currentSize; compactedContentSignature = currentSig; compactedTranscriptCache = compactTranscript(raw);
    }

    private static long rollingSignature(List<TranscriptEntry> raw) {
        if (raw == null || raw.isEmpty()) return 0L; long h = 1469598103934665603L;
        for (TranscriptEntry e : raw) { if (e == null) { h = fnv1a(h, 0); continue; } h = fnv1a(h, EntryKey.of(e).hashCode()); }
        return fnv1a(h, raw.size());
    }

    private static final class EntryKey {
        private final String role, speaker, text, nodeId, sayId, choiceId; private final int choiceIndex, hash;
        private EntryKey(String role, String speaker, String text, String nodeId, String sayId, String choiceId, int choiceIndex) {
            this.role = role; this.speaker = speaker; this.text = text; this.nodeId = nodeId; this.sayId = sayId; this.choiceId = choiceId; this.choiceIndex = choiceIndex;
            int h = 17; h = 31 * h + role.hashCode(); h = 31 * h + speaker.hashCode(); h = 31 * h + text.hashCode(); h = 31 * h + nodeId.hashCode(); h = 31 * h + sayId.hashCode(); h = 31 * h + choiceId.hashCode(); this.hash = 31 * h + choiceIndex;
        }
        static EntryKey of(TranscriptEntry e) { return new EntryKey(normFast(e.role()), normFast(e.speaker()), normFast(e.text()), normFast(e.nodeId()), normFast(e.sayId()), normFast(e.choiceId()), e.choiceIndex() == null ? Integer.MIN_VALUE : e.choiceIndex()); }
        @Override public int hashCode() { return hash; }
        @Override public boolean equals(Object obj) { if (this == obj) return true; if (!(obj instanceof EntryKey other)) return false; return choiceIndex == other.choiceIndex && role.equals(other.role) && speaker.equals(other.speaker) && text.equals(other.text) && nodeId.equals(other.nodeId) && sayId.equals(other.sayId) && choiceId.equals(other.choiceId); }
    }

    private static String normFast(String s) {
        if (s == null || s.isEmpty()) return ""; StringBuilder out = new StringBuilder(s.length()); boolean prevSpace = false;
        for (int i = 0; i < s.length(); i++) { char c = s.charAt(i); if (Character.isWhitespace(c)) { if (!prevSpace) { out.append(' '); prevSpace = true; } } else { out.append(Character.toLowerCase(c)); prevSpace = false; } }
        if (out.length() > 0 && out.charAt(0) == ' ') out.deleteCharAt(0); if (out.length() > 0 && out.charAt(out.length() - 1) == ' ') out.deleteCharAt(out.length() - 1); return out.toString();
    }

    private static long fnv1a(long hash, String s) { for (int i = 0; i < s.length(); i++) { hash ^= s.charAt(i); hash *= 1099511628211L; } return hash; }
    private static long fnv1a(long hash, int v) { hash ^= (v) & 0xFF; hash *= 1099511628211L; hash ^= (v >>> 8) & 0xFF; hash *= 1099511628211L; hash ^= (v >>> 16) & 0xFF; hash *= 1099511628211L; return (hash ^ ((v >>> 24) & 0xFF)) * 1099511628211L; }
    private static List<TranscriptEntry> getCompactedTranscript(List<TranscriptEntry> raw) { ensureCompactedTranscriptUpToDate(raw); return compactedTranscriptCache; }
}