...
public class TradeCategoryPanel {
    ...
    public void render(GuiGraphics g, int lx, int ly, int lw, int lh, int mx, int my, float dt, float alpha, boolean isClosing, float fastClose) {
        selectedCatSlide = ArcAnimClock.lerp(selectedCatSlide, selectedCategoryIndex, 0.2f, dt);
        int hlY = ly + 10 + (int) (selectedCatSlide * 36);
        g.fill(lx + 4, hlY, lx + 7, hlY + 28, ArcDrawUtil.withAlpha(selectedCategoryIndex == 0 ? screen.getShop().getThemeColor() : (screen.getShop().getCategories().isEmpty() ? screen.getShop().getThemeColor() : screen.getShop().getCategories().get(Math.max(0, selectedCategoryIndex - 1)).getThemeColor()), (int) (255 * alpha)));
    ...
    private void drawCatRow(GuiGraphics g, int x, int y, int w, String text, boolean sel, int idx, int mx, int my, float dt, float alpha, boolean isClosing, float fastClose) {
        ...
        catHoverAnims[idx] = ArcAnimClock.step(catHoverAnims[idx], hov ? 1f : 0f, 8f, dt);
        float hEase = ArcAnimClock.easeOutCubic(catHoverAnims[idx]), contentScale = isClosing ? ArcAnimClock.easeInCubic(fastClose) : 1.0f;
        ...
        if (contentScale > 0.01f) {
            ...
            g.pose().translate(-(x + w / 2f), -(y + 14), 0);
            g.drawString(font, text, textX, y + 10, ArcDrawUtil.withAlpha((c << 16) | (c << 8) | c, (int) (255 * alpha)), true);
            g.pose().popPose();
        }
    }
