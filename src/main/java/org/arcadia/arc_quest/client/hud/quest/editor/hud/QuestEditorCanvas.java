...
public class QuestEditorCanvas {
    ...
    private boolean rightClick(QuestEditorLayoutEnvelope env, QuestEditorState s, String node) {
        ...
        if (controller.quest().layout.phasePositions.get(node) == null && p != null)
            controller.moveSelectedPhaseNode(p.x, p.y);
        return true;
    }
    ...
    private boolean clickNode(QuestEditorLayoutEnvelope env, QuestEditorState s, String node, double wx, double wy) {
        if (s.mode == QuestEditorMode.DELETE_PHASE_SELECT) {
            if (s.pendingDeletePhaseNodeIds.contains(node)) controller.removePhaseFromPendingDelete(node);
            else controller.addPhaseToPendingDelete(node);
            return true;
        }
        if (s.mode == QuestEditorMode.CONNECT_GOTO) {
            controller.finishGotoConnection(node);
            screen.refreshLayout();
            return true;
        }
        var p = env.graphLayout.nodePositions.get(node);
        ...
        if (controller.selectedPhaseNodeIds().contains(node)) controller.setSelection(null);
        else controller.selectPhaseByNodeId(node);
        return true;
    }
    ...
    private boolean clickCollection(CollectionLayout l, QuestEditorState s, double wx, double wy) {
        ...
        if (hitEntry != null) {
            if (s.mode == QuestEditorMode.DELETE_COLLECTION_OBJECT_SELECT)
                controller.addCollectionObjectToPendingDelete(EditorObjectRef.entry(hitEntry.entryId));
            else {
        ...
        if (hitCat != null) {
            if (s.mode == QuestEditorMode.DELETE_COLLECTION_OBJECT_SELECT)
                controller.addCollectionObjectToPendingDelete(EditorObjectRef.category(hitCat.categoryId));
            else {
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        ...
        if (isRightDraggingNode && draggingNodeId != null && button == 1) {
            ...
            if (p != null) {
                ...
                controller.moveSelectedPhaseNode(Math.max(0, p.x + dragX / s.zoom), Math.max(0, p.y + dragY / s.zoom));
                screen.refreshLayout();
                return true;
            }
        }
