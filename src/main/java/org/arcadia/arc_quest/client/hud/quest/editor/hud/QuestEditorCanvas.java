...
public class QuestEditorCanvas {
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        if ("PROGRESSION".equals(l.mode) && l.graphLayout != null) renderProgress(g, font, l.graphLayout, wx, wy);
        else if (l.collectionLayout != null) renderCollection(g, font, l.collectionLayout, wx, wy);
        if (s.mode == QuestEditorMode.CONNECT_GOTO && s.connectSourcePhaseNodeId != null && l.graphLayout != null) {
            ...
            if (p != null) drawLine(g, (int) (p.x + NODE_W), (int) (p.y + NODE_H / 2), (int) wx, (int) wy, 0xFFFFCC00);
        }
        g.pose().popPose();
    }
    ...
    private void renderProgress(GuiGraphics g, Font font, QuestEditorGraphLayout l, double mx, double my) {
        for (EditorEdgeLayout e : l.edges) {
            var a = l.nodePositions.get(e.sourceNodeId); var b = l.nodePositions.get(e.targetNodeId);
            if (a != null && b != null) drawLine(g, (int) (a.x + NODE_W), (int) (a.y + NODE_H / 2), (int) b.x, (int) (b.y + NODE_H / 2), "CHOICE".equals(e.edgeType) ? 0xFFFFAA00 : 0xFF5AD7FF);
        }
        for (Map.Entry<String, org.arcadia.arc_quest.quest.editor.model.EditorNodePosition> e : l.nodePositions.entrySet()) {
            String id = e.getKey(); var p = e.getValue(); int x = (int) p.x, y = (int) p.y;
    ...
    private void renderCollection(GuiGraphics g, Font font, CollectionLayout l, double mx, double my) {
        ...
        for (CollectionCategoryLayout c : l.categories) {
            ...
            if (!c.collapsed) for (CollectionEntryLayout e : c.entries) renderEntry(g, font, e, mx, my);
        }
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (s.mode == QuestEditorMode.PLACE_PHASE) { controller.placePhaseAt(wx, wy); screen.refreshLayout(); return true; }
        if (node != null) return clickNode(env, s, node, wx, wy);
        if ("COLLECTION".equals(env.mode) && env.collectionLayout != null && clickCollection(env.collectionLayout, s, wx, wy)) return true;
    ...
    private boolean rightClick(QuestEditorLayoutEnvelope env, QuestEditorState s, String node) {
        if (s.mode == QuestEditorMode.PLACE_PHASE) { controller.cancelPlacePhase(); return true; }
        if (node == null) return false;
        controller.selectPhaseByNodeId(node); isRightDraggingNode = true; draggingNodeId = node;
        var p = env.graphLayout.nodePositions.get(node);
        if (controller.quest().layout.phasePositions.get(node) == null && p != null) controller.moveSelectedPhaseNode(p.x, p.y);
        return true;
    }

    private boolean clickNode(QuestEditorLayoutEnvelope env, QuestEditorState s, String node, double wx, double wy) {
        if (s.mode == QuestEditorMode.DELETE_PHASE_SELECT) { if (s.pendingDeletePhaseNodeIds.contains(node)) controller.removePhaseFromPendingDelete(node); else controller.addPhaseToPendingDelete(node); return true; }
        if (s.mode == QuestEditorMode.CONNECT_GOTO) { controller.finishGotoConnection(node); screen.refreshLayout(); return true; }
        var p = env.graphLayout.nodePositions.get(node);
        boolean edit = controller.selectedPhaseNodeIds().contains(node) && inside(wx, wy, p.x + NODE_W - 70, p.y + NODE_H - 24, 62, 18);
        if (edit) { screen.openModal(QuestEditorScreen.ModalType.PHASE_EDIT); return true; }
        if (controller.selectedPhaseNodeIds().contains(node)) controller.setSelection(null); else controller.selectPhaseByNodeId(node);
        return true;
    }

    private boolean clickCollection(CollectionLayout l, QuestEditorState s, double wx, double wy) {
        CollectionEntryLayout hitEntry = null; CollectionCategoryLayout hitCat = null;
        ...
        if (hitEntry != null) { if (s.mode == QuestEditorMode.DELETE_COLLECTION_OBJECT_SELECT) controller.addCollectionObjectToPendingDelete(EditorObjectRef.entry(hitEntry.entryId)); else { screen.editingEntryId = hitEntry.entryId; screen.openModal(QuestEditorScreen.ModalType.ENTRY_EDIT); } return true; }
        if (hitCat != null) { if (s.mode == QuestEditorMode.DELETE_COLLECTION_OBJECT_SELECT) controller.addCollectionObjectToPendingDelete(EditorObjectRef.category(hitCat.categoryId)); else { screen.editingCategoryId = hitCat.categoryId; screen.openModal(QuestEditorScreen.ModalType.CATEGORY_EDIT); } return true; }
        return false;
    }
    ...
    private String hitNode(QuestEditorLayoutEnvelope env, double wx, double wy) {
        ...
        for (var e : env.graphLayout.nodePositions.entrySet()) if (inside(wx, wy, e.getValue().x, e.getValue().y, NODE_W, NODE_H)) return e.getKey();
        return null;
    }
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        ...
        if (isRightDraggingNode && draggingNodeId != null && button == 1) {
            var p = controller.quest().layout.phasePositions.get(draggingNodeId);
            if (p != null) { controller.selectPhaseByNodeId(draggingNodeId); controller.moveSelectedPhaseNode(Math.max(0, p.x + dragX / s.zoom), Math.max(0, p.y + dragY / s.zoom)); screen.refreshLayout(); return true; }
        }
        if (isPanning && button == 0) { s.cameraX -= dragX / s.zoom; s.cameraY -= dragY / s.zoom; return true; }
        return false;
    }

    public boolean mouseReleased(double mouseX, double mouseY, int button) { isPanning = false; isRightDraggingNode = false; draggingNodeId = null; return false; }

    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        if (mouseY < 30 || mouseX > screen.width - 140 || !screen.hasActiveQuest()) return false;
        QuestEditorState s = controller.state(); double old = s.zoom;
        s.zoom = Math.max(0.2, Math.min(3.0, s.zoom + delta * 0.1));
        s.cameraX += (mouseX - screen.width / 2.0) * (1 / old - 1 / s.zoom);
        s.cameraY += (mouseY - screen.height / 2.0) * (1 / old - 1 / s.zoom);
        return true;
    }

    private void drawLine(GuiGraphics g, int x1, int y1, int x2, int y2, int color) { int dx = x2 - x1, dy = y2 - y1, steps = Math.max(Math.abs(dx), Math.abs(dy)); if (steps == 0) return; float xi = dx / (float) steps, yi = dy / (float) steps, x = x1, y = y1; for (int i = 0; i <= steps; i++) { g.fill((int) x, (int) y, (int) x + 2, (int) y + 2, color); x += xi; y += yi; } }
    private void outline(GuiGraphics g, int x, int y, int w, int h, int color) { g.fill(x, y, x + w, y + 1, color); g.fill(x, y + h - 1, x + w, y + h, color); g.fill(x, y, x + 1, y + h, color); g.fill(x + w - 1, y, x + w, y + h, color); }
}
