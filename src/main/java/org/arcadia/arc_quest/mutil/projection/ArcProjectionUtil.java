package org.arcadia.arc_quest.mutil.projection;
...
public final class ArcProjectionUtil {
    private ArcProjectionUtil() {
    }
    ...
    public static ScreenResult project(double worldX, double worldY, double worldZ, float edgePadding) {
        ...
        double fovDynamic = ((MixinGameRendererAccessor) mc.gameRenderer).arcQuest$invokeGetFov(camera, partialTick, true);
        ...
        if (inFrontSafe && !behind) {
            ...
            boolean onScreen = sx >= edgePadding && sx <= (screenW - edgePadding) && sy >= edgePadding && sy <= (screenH - edgePadding);
            if (onScreen) {
        ...
        if (edge[1] > bottomYThreshold && Math.abs(edge[0] - halfW) < hotbarHalfW) {
            edge[0] = edge[0] >= halfW ? halfW + hotbarHalfW : halfW - hotbarHalfW;
            dirX = edge[0] - halfW;
            dirY = edge[1] - halfH;
        }
        if (behind) {
    ...
    private static float[] clampDirectionToEllipse(float dirX, float dirY, float cx, float cy, float padding, int screenW, int screenH) {
