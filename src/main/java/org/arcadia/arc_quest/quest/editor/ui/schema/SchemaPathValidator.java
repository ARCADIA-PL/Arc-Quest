...
public class SchemaPathValidator {
    ...
    public List<String> validate() {
        ...
        for (FormSchema form : registry.all().values()) {
            ...
            for (FieldSchema field : form.fields) {
                ...
                if ("phase.objective".equals(form.formId) && ("collection.categoryId".equals(field.key) || "condition.compareOp".equals(field.key)))
                    continue;
    ...
    private Object sampleTarget(String objectType) {
        if ("OBJECTIVE".equalsIgnoreCase(objectType))
            return new org.arcadia.arc_quest.quest.editor.model.EditableObjective();
        if ("CONNECTION".equalsIgnoreCase(objectType))
            return new org.arcadia.arc_quest.quest.editor.model.EditableConnection();
        if ("REWARD_NODE".equalsIgnoreCase(objectType))
            return new org.arcadia.arc_quest.quest.editor.model.EditableCollectionRewardNode();
        return null;
    }
}
