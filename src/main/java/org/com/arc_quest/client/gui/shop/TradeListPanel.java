...
public class TradeListPanel {
    ...
    public void clampScroll(int listHeight) {
        int maxScroll = Math.max(0, screen.getFilteredEntries().size() * (CARD_HEIGHT + 8) - listHeight);
    ...
    public void render(GuiGraphics g, int rx, int ry, int rw, int rh, int mx, int my, float dt, float alpha, boolean isClosing, float fastClose) {
        clampScroll(rh); scrollOffset += (targetScroll - scrollOffset) * Math.min(1.0, dt * 12.0);
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            boolean onCd = cache.isEntryCoolingDown(screen.getShopId(), gi, entry), maxed = cache.isPurchaseLimitReached(screen.getShopId(), gi, entry);
            boolean locked = !onCd && !maxed && cache.isConditionBlocked(screen.getShopId(), gi, entry), canBuy = !onCd && !maxed && !locked;
            ...
            int bgA = (int) ((0x22 + 0x33 * hEase) * alpha), bdA = (int) ((0x44 + 0x66 * hEase) * alpha);
            ...
            if (gi == screen.getLastClickedGi()) {
                if (screen.isFeedbackSuccess() && screen.getFeedbackAnim() > 0) {
                    bdA = Math.min(255, bdA + (int)(180 * screen.getFeedbackAnim() * alpha)); bRgb = screen.lerpColor(bRgb, 0x55FF55, screen.getFeedbackAnim());
                    ...
                    bdA = Math.min(255, bdA + (int)(180 * intensity * alpha)); bRgb = screen.lerpColor(bRgb, 0xFF3333, intensity);
                }
            }
            ...
            if (contentScale > 0.01f) {
                g.pose().pushPose(); g.pose().translate(cx + cw/2f, cy + ch/2f, 0); float aScale = contentScale + hEase * 0.02f; g.pose().scale(aScale, aScale, 1f); g.pose().translate(-(cx + cw/2f), -(cy + ch/2f), 0);

                if (entry.getRewardIcon() != null) screen.drawAdaptiveIcon(g, entry.getRewardIcon(), cx + 7, cy + 16, 16, 16, alpha);
                else { ItemStack is = screen.getIconStackForEntry(entry); if (!is.isEmpty()) { g.pose().pushPose(); g.pose().translate(cx + 12, cy + 16, 0); g.pose().scale(1.2f, 1.2f, 1f); g.renderItem(is, 0, 0); g.pose().popPose(); } }

                if (onCd || maxed || locked) {
                    int pA = (int) (255 * (0.6f + 0.4f * (float)(Math.sin(Util.getMillis()/200.0)*0.5+0.5)) * alpha);
                    int pC = onCd ? 0xFF6666 : (maxed ? 0xAAAAAA : 0x4488CC);
                    g.fill(cx, cy, cx + cw, cy + ch, HudAnimUtil.withAlpha(pC, (int)(pA * 0.1f)));
                    g.fill(cx - 1, cy - 1, cx + cw + 1, cy, HudAnimUtil.withAlpha(pC, pA)); g.fill(cx - 1, cy + ch, cx + cw + 1, cy + ch + 1, HudAnimUtil.withAlpha(pC, pA));
                    g.fill(cx - 1, cy, cx, cy + ch, HudAnimUtil.withAlpha(pC, pA)); g.fill(cx + cw, cy, cx + cw + 1, cy + ch, HudAnimUtil.withAlpha(pC, pA));
                    g.fill(cx, cy, cx + cw, cy + ch, HudAnimUtil.withAlpha(0x000000, (int)(160 * alpha)));
                }

                int textX = cx + 52 + (int)(4 * hEase);
                String statStr = onCd ? cache.getCooldownText(screen.getShopId(), gi) : (maxed ? Component.translatable("arc_quest.gui.trade.status.maxed").getString() : (locked ? Component.translatable("arc_quest.gui.trade.status.locked").getString() : ""));
                ...
                for (int j = 0; j < entry.getCosts().size(); j++) {
                    ...
                    if (j > 0) { g.drawString(font, "+", cX, costY, HudAnimUtil.withAlpha(0x777777, (int)(255*alpha)), true); cX += font.width("+") + 2; }
                    g.pose().pushPose(); g.pose().translate(cX, costY - 1, 0); g.pose().scale(0.6f, 0.6f, 1f);
                    if (cost.getIcon() != null) screen.drawAdaptiveIcon(g, cost.getIcon(), 0, 0, 16, 16, alpha);
                    else { ItemStack cS = screen.getIconStackForOffer(cost); if (!cS.isEmpty()) g.renderItem(cS, 0, 0); }
                    g.pose().popPose(); cX += 12;

                    String cDesc = cost.describe().getString();
                    g.drawString(font, cDesc, cX, costY, HudAnimUtil.withAlpha(canBuy ? 0xDDDDDD : 0x777777, (int)(255*alpha)), true); cX += font.width(cDesc) + 4;
                    if (cX > cx + cw - 100) { g.drawString(font, "...", cX, costY, HudAnimUtil.withAlpha(0x777777, (int)(255*alpha)), true); break; }
                }

                int btnW = 80, btnH = 24, btnX = cx + cw - btnW - 12, btnY = cy + (ch - btnH) / 2;
                String btnText = canBuy ? Component.translatable("arc_quest.gui.trade.btn.purchase").getString() : (onCd ? Component.translatable("arc_quest.gui.trade.btn.wait").getString() : (locked ? Component.translatable("arc_quest.gui.trade.btn.locked").getString() : Component.translatable("arc_quest.gui.trade.btn.empty").getString()));
                int btnC = canBuy ? ((hov && mx >= btnX && mx < btnX + btnW && my >= btnY && my < btnY + btnH) ? 0xFFFFFF : screen.getThemeColorForEntry(entry)) : (locked ? 0x4488CC : 0x888888);
        ...
        int maxScroll = Math.max(0, entries.size() * (CARD_HEIGHT + 8) - rh);
        if (maxScroll > 0) {
            int th = Math.max(16, (int) (((float) rh / (entries.size() * (CARD_HEIGHT + 8))) * rh)), ty = ry + (int) ((scrollOffset / maxScroll) * (rh - th));
    ...
    public TradeEntry getHoveredEntry(int mx, int my, int rx, int ry, int rw, int rh) {
        if (mx >= rx && mx < rx + rw && my >= ry && my <= ry + rh) {
            ...
            if (vi >= 0 && vi < screen.getFilteredEntries().size() && my >= ry + (int) (vi * (CARD_HEIGHT + 8) - scrollOffset) + 8 && my < ry + (int) (vi * (CARD_HEIGHT + 8) - scrollOffset) + 8 + CARD_HEIGHT) return screen.getFilteredEntries().get(vi);
        }
        return null;
    }
}