...
public abstract class AbstractTradeScreen extends Screen {
    ...
    public void onTradeSuccess() {
        feedbackSuccess = true; feedbackAnim = 1f;
    ...
    private TooltipData calcTooltipData(TradeEntry entry, int gi, int mx, int my) {
        ...
        d.x = mx + 16;
        d.y = my + 16;
        if (d.x + d.w > width) d.x = mx - d.w - 8;
        if (d.y + d.h > height) d.y = my - d.h - 8;
        return d;
    }
    ...
    private void renderMorphingTooltip(GuiGraphics g, TradeEntry entry, int gi, int mx, int my) {
        ...
        if (animBgW == 0) {
            ...
            animThemeColor = lerpColor(animThemeColor, target.themeColor, Math.min(1f, dt * 15f));
        }
        ...
        int drawX = (int)animBgX; int drawY = (int)animBgY;
        int drawW = (int)animBgW; int drawH = (int)animBgH;

        g.pose().pushPose();
        g.pose().translate(0, 0, 400);

        
        g.pose().translate(drawX + drawW / 2f, drawY + drawH / 2f, 0);
        g.pose().scale(scale, scale, 1f);
        g.pose().translate(-(drawX + drawW / 2f), -(drawY + drawH / 2f), 0);

        int bgA = (int) (0xDD * scale);
        int borderA = (int) (0xFF * scale);
        int borderColor = (borderA << 24) | (animThemeColor & 0xFFFFFF);

        g.fill(drawX, drawY, drawX + drawW, drawY + drawH, (bgA << 24) | 0x0A0A10);
        g.fill(drawX - 1, drawY - 1, drawX + drawW + 1, drawY, borderColor);
        g.fill(drawX - 1, drawY + drawH, drawX + drawW + 1, drawY + drawH + 1, borderColor);
        g.fill(drawX - 1, drawY, drawX, drawY + drawH, borderColor);
        g.fill(drawX + drawW, drawY, drawX + drawW + 1, drawY + drawH, borderColor);

        g.enableScissor(drawX, drawY, drawX + drawW, drawY + drawH);

        
        float contentSlideY = (1f - scale) * 8f;
        g.pose().pushPose();
        g.pose().translate(0, contentSlideY, 0);
        ...
        if (target.maxP > 0) {
            String limitStr = "限购 / Limit: " + target.purchases + " / " + target.maxP;
            g.drawString(font, limitStr, drawX + padding, currentY, QuestAnimUtil.withAlpha(0xDDDDDD, safeAlpha), true);
            currentY += font.lineHeight + 2;

            float progress = Math.min(1f, (float)target.purchases / target.maxP);
            ...
            int barColor = progress >= 1f ? 0xAA3333 : (progress >= 0.8f ? 0xDD9933 : 0x33AA33);
            if (progress > 0) {
                g.fill(drawX + padding, currentY, drawX + padding + (int)(barW * progress), currentY + barH, QuestAnimUtil.withAlpha(barColor, safeAlpha));
            }
            currentY += barH + 4;
        }
        ...
        if (target.onCd) {
            String cdText = ClientCooldownHelper.getCooldownText(
                    ...
                    cooldownTypes[gi], cooldownValues[gi], resetTimeTicks[gi]);
            g.drawString(font, "冷却 / Cooldown: " + cdText, drawX + padding, currentY, QuestAnimUtil.withAlpha(0xFF5555, safeAlpha), true);
        }
