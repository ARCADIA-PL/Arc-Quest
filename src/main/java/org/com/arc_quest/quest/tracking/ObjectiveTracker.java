...
public final class ObjectiveTracker {
     ...
     */
    private final Map<ObjectiveKey, Set<TrackedObjective>> index = new HashMap<>();

    /**
     * 反向索引：playerId → 该玩家所有正在追踪的句柄（用于快速移除）
     */
    private final Map<UUID, Set<TrackedObjective>> byPlayer = new HashMap<>();
    ...
    public void registerPhase(UUID playerId, ResourceLocation questId, String phaseId) {
        ...
        for (int i = 0; i < objectives.size(); i++) {
            ...
            this.index.computeIfAbsent(key, k -> new LinkedHashSet<>()).add(tracked);
            this.byPlayer.computeIfAbsent(playerId, k -> new LinkedHashSet<>()).add(tracked);
        }
    ...
    public void unregisterPhase(UUID playerId, ResourceLocation questId, String phaseId) {
        Set<TrackedObjective> playerSet = this.byPlayer.get(playerId);
        ...
        while (it.hasNext()) {
            ...
            if (tracked.getQuestId().equals(questId) && tracked.getPhaseId().equals(phaseId)) {
                it.remove();
                // 同步从主索引移除
                Set<TrackedObjective> keySet = this.index.get(tracked.getKey());
                if (keySet != null) {
    ...
    public void unregisterPlayer(UUID playerId) {
        Set<TrackedObjective> playerSet = this.byPlayer.remove(playerId);
        if (playerSet == null) return;
    ...
    public void register(TrackedObjective tracked) {
        ...
        this.index.computeIfAbsent(key, k -> new LinkedHashSet<>()).add(tracked);
        this.byPlayer.computeIfAbsent(tracked.getPlayerId(), k -> new LinkedHashSet<>()).add(tracked);
    }
    ...
    public void unregister(TrackedObjective tracked) {
        ...
        Set<TrackedObjective> playerSet = this.byPlayer.get(tracked.getPlayerId());
        if (playerSet != null) {
    ...
    public Set<TrackedObjective> lookup(ObjectiveKey key) {
        Set<TrackedObjective> result = this.index.get(key);
    ...
    public List<TrackedObjective> lookup(UUID playerId, ObjectiveKey key) {
        Set<TrackedObjective> all = this.index.get(key);
    ...
    public int getTrackedCount() {
        int count = 0;
        for (Set<TrackedObjective> set : this.index.values()) {
            count += set.size();
        }
        return count;
    }
